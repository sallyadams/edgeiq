...
- **Stripe**: €9/month Early, €19/month Pro, and €49/month Elite subscriptions via `/api/checkout/create-session`. Uses Replit Stripe integration (connector) for keys — auto-selects test/live based on environment. Webhook at `/api/webhook/stripe` handles: `checkout.session.completed` (upgrades user tier + stores Stripe customer/subscription IDs), `invoice.payment_succeeded` (confirms recurring payments), `customer.subscription.created` (links subscription), `customer.subscription.deleted` (downgrades to free). Signature verification via `STRIPE_WEBHOOK_SECRET` (required in production, optional in dev). User tier persisted in `users.tier`, Stripe IDs in `users.stripeCustomerId` and `users.stripeSubscriptionId`. Subscription metadata propagated to Stripe for traceability. Checkout redirects to `/payment/success` and `/payment/cancel` pages.
...
- **Activation Flow Components**:
  ...
  - `SignalCard.tsx` — Enhanced signal cards with: risk level badge (Low/Medium/High), signal status (Active/Closed/Expired based on age), entry zone, timeframe, conviction score ring, AI insights (locked/blurred for free users), and CTA buttons row (Track Trade, Trade Now, Ask Assistant).
  - `ChatWidget.tsx` — Floating AI assistant widget (bottom-right). Predefined flows: Quick Tour, Explain Signals, How to Trade, Paper Trading Help, Upgrade to Pro. Context-aware keyword matching for freeform questions. Markdown rendering for rich assistant responses.
  - `OnboardingModal.tsx` — Multi-step onboarding for first-time users (5 steps: Welcome, AI Signals, Trade Tracking, Paper Trading, Get Started). Persisted in localStorage (`edgeiq_onboarding_seen`). Auto-shows on first Dashboard visit.
  - `BrokerConnect.tsx` — Broker connection placeholder modal showing Alpaca (coming soon), Interactive Brokers (coming soon), Tradier (planned). Links to paper trading as immediate alternative. Accessible from TradeModal "Connect broker for live trading" link.
- **Layout sidebar**: Shows "Free Plan" badge and "Unlock All Signals" CTA button for non-unlocked users.
- **Landing page**: Full conversion-optimized landing with: hero + strong CTA, stats bar, "How it works in 30 seconds" (4 steps), feature grid, "Who is EdgeIQ for?" (3 personas: Beginners, Busy Professionals, Data-Driven Traders), founder trust section (Salamat Adams), 3-tier pricing cards, early access urgency section, final CTA, and risk disclaimer footer. All copy translated across 5 languages.
...
- **Payment Pages**: `/payment/success` (animated confirmation with feature unlock list, CTA to signals/dashboard) and `/payment/cancel` (reassurance message, CTA to free access). Both fully translated in 5 languages.
- **i18n keys**: Full activation flow, trading, and payment pages translated in all 5 locales — `upgradeModal.*`, `signalCard.aiInsight/upgradeToUnlock`, `dashboard.*`, `trading.*`, `payment.*` (success/cancel pages).
